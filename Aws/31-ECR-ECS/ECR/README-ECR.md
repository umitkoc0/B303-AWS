# Hands-on ECR : Pushing Docker Images to ECR Manually
...
- Enter a repository name ex. `techpro-repo/todo-app` (***In this hands-on, we will be working with a simple `todo list manager` that is running in `Node.js`. If you're not familiar with Node.js, don't worry! No real JavaScript experience is needed!***)
...
```text
xxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app
```
...
```bash
docker build -t techpro-repo/todo-app .
```
...
```bash
docker run --name todo -d -p 80:3000 techpro-repo/todo-app
```
...
```bash
docker tag todo-app:latest xxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app:latest
```

- Push the image to your ECR repository and show the pushed image on AWS ECR Console.

```bash
docker push xxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app:latest
```

- Create an container from image located ECR

```bash
docker pull xxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app:latest
docker run --name todo -dp 80:3000 xxxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app:latest
```
- Stop and remove container
...
```bash
docker run --name todo-v1 -dp 80:3000 xxxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro-repo/todo-app:latest
```
...
- Create a ECR repository named `techpro2-repo/todo-app` from AWS CLI and show it is created on AWS ECR Console.

```bash
aws ecr create-repository \
    --repository-name techpro2-repo/todo-app \
    --image-scanning-configuration scanOnPush=false \
    --image-tag-mutability IMMUTABLE \
    --region us-east-1
```

- Tag and push the image to your `techpro2-repo/todo-app` repository and show the pushed image on AWS ECR Console.

```bash
docker tag techpro-repo/todo-app:latest xxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro2-repo/todo-app:latest
docker push xxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro2-repo/todo-app:latest
```
...
- Pull the image from your `techpro2-repo/todo-app` repository  to the local.

```bash
docker pull xxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro2-repo/todo-app:latest
docker run -dp 80:3000 xxxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro2-repo/todo-app:latest
```

- Or directly run the `todo-app` from the ECR repo.

```bash
docker run -dp 80:3000 xxxxxxxxxxxxx.dkr.ecr.us-east-1.amazonaws.com/techpro2-repo/todo-app:latest
```

- Check if the To-Do App is running by entering `http://<ec2-host-name>` in a browser.

- Delete Docker image on your `techpro2-repo/todo-app` repository from AWS CLI. Since we didn't configure aws CLI you may get error. If you can't do the process with root, then exit from root.
...
- Delete the ECR repository  `techpro2-repo/todo-app` from AWS CLI.
...
- Delete the image and repository of `techpro-repo/todo-app` from AWS ECR Console.
