...
- Create a `techpro-pv.yaml` file using the following content with the volume type of `hostPath` to build a `PersistentVolume` and explain fields.
...
- Create the PersistentVolume `techpro-pv-vol`.

```bash
kubectl apply -f techpro-pv.yaml
```
...
- Create a `techpro-pv-claim.yaml` file using the following content to create a `PersistentVolumeClaim` and explain fields.
...
- Create the PersistentVolumeClaim `techpro-pv-claim`.
...
- View information about the `PersistentVolumeClaim` and show that the `PersistentVolumeClaim` is bound to your PersistentVolume `techpro-pv-vol`.

```bash
kubectl get pvc techpro-pv-claim
```
...
- Create a `techpro-pod.yaml` file that uses your PersistentVolumeClaim as a volume using the following content.
...
spec:
  volumes:
    - name: techpro-pv-storage
      persistentVolumeClaim:
        claimName: techpro-pv-claim
  containers:
    - name: techpro-pv-container
      image: nginx
      ports:
        - containerPort: 80
          name: "http-server"
...
- Create the Pod `techpro-pod`.

```bash
kubectl apply -f techpro-pod.yaml
```

- Verify that the Pod is running.

```bash
kubectl get pod techpro-pod
kubectl describe po techpro-pod
```

- Open a shell to the container running in your Pod.

```bash
kubectl exec -it techpro-pod -- /bin/bash
```
...
```bash
kubectl expose pod techpro-pod --port=80 --type=NodePort
```
...
```bash
kubectl delete pod techpro-pod
kubectl delete pvc techpro-pv-claim
kubectl delete pv techpro-pv-vol
```
...
```bash
sudo ctr --namespace k8s.io tasks kill <container-id>
```
