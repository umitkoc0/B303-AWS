...
Launch an AWS EC2 instance of Amazon Linux 2023 AMI with instance type t3a.medium and a security group allowing SSH.
...
3. Select ```Cluster``` on the left-hand menu and click on `Create cluster` button. You will be directed to the ```Configure cluster``` page:
    ...
    - Fill the ```Name``` and ```Kubernetes version``` fields. (Ex: MyCluster, 1.32)
       
       (Ex: MyCluster, 1.32)

        <i>Mention the durations for minor version support and the approximate release frequency.</i>

    - On the ```Cluster IAM role``` field; give general description about why we need this role. 
...
5. On the ```Configure observability``` page:
...
5. On the ```Set compute and scaling configuration``` page:
    ...
    - Choose the appropriate AMI type for Non-GPU instances. (Amazon Linux 2023 (AL2_x86_64))
...
1. Explain what ```Cluster Autoscaler``` is and why we need it.
   ...
   Cluster Autoscaler >> Add permissions >> creat inline policy >> json
...
This command will open the yaml file for your editting. Replace <CLUSTER NAME> value with your own cluster name, and add the following command option
    <CLUSTER NAME>
 ```--skip-nodes-with-system-pods=false``` to the command section under ```containers``` under ```spec```. Save and exit the file by pressing ```:wq```. The changes will be applied.

7. Find an appropriate version of your cluster autoscaler in the [link](https://github.com/kubernetes/autoscaler/releases). The version number should start with version number of the cluster Kubernetes version. For example, if you have selected the Kubernetes version 1.32, you should find something like ```v1.32.1```.
...
```bash
kubectl -n kube-system set image deployment.apps/cluster-autoscaler cluster-autoscaler=k8s.gcr.io/autoscaling/cluster-autoscaler:v1.32.1
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: container-info
        image: techprodevops348/hellotech
        ports:
        - containerPort: 80
```
...
```bash
kubectl -n my-namespace get svc,deploy,po
```
...
12. Atfer clean-up `worker nodes` and `cluster`, delete the `LoadBalancer` and `ec2` manually. 