...
- Launch a t3a.medium (Nexus needs 8 GB of RAM) EC2 instance using the Amazon Linux 23 AMI (ami-03c7d01cf4dedc891) with security group allowing `SSH (22), Nexus Port (8081), 8082 and 8083` connections.
...
wget: komutu, ağ üzerinden dosyaları indirmek için kullanılan bir komut satırı programıdır.
 Bu araç, HTTP, HTTPS ve FTP gibi protokoller üzerinden dosya transferi yapabilme yeteneğine sahiptir
```bash
cd /opt
sudo wget -O nexus.tar.gz https://download.sonatype.com/nexus/3/nexus-3.80.0-06-linux-aarch_64.tar.gz
ls
```
...
chown nexus root >>> ec3-user olr kulm
R hps 
```bash
ll
...
ll
```
It is generally used to specify that a script or application should be run under a specific user identity (in this case, "ec2-user").
...
code /home/ec2-user/.m2/settings.xml
```
...
- Add the distributionManagement element given below to your pom.xml file after the `</dependencies>` altn line. Include the endpoints to your maven-releases and maven-snapshots repos. Change localhost >>>>  Public IPv4 address of your server.
17, 22
```
```
localhost = Public IPv4 address
```
...
>> snapshopts, releases, proxy-hans-on
...
### For the blob storage
 blob storage: local repo

- Click the config (gear) icon.
- Navigate to 'Blob Stores'.>> Create a new Blob Store 
...
Add the 'Docker Bearer Token Realm'  2 Click
Click 'Save'

### For the repo   Click 'Repositories'
...
daemon.json dosyası, Docker daemon'ın yapılandırma ayarlarını içeren bir JSON formatında dosyadır. Docker daemon, Docker konteynerlerini çalıştırmak ve yönetmek için arka planda çalışan bir süreçtir. daemon.json dosyası, Docker'ın nasıl davranacağını özelleştirmek için kullanılır ve Docker servisinin başlatılması sırasında okunur.
...
```bash
docker login -u admin -p admin 44.200.32.193:8082 # don't forget to change password.
docker image ls
docker pull 44.200.32.193:8082/alpine
docker image ls
```
...
```bash
docker login -u admin -p admin 44.223.16.90:8083
