...
resource "aws_s3_bucket" "tf-remote-state" {
  bucket = "tf-remote-s3-bucket-techpro-changehere"
...
resource "aws_s3_bucket" "tf-test-1" {
  bucket = "techpro-test-1-versioning"
}
```
...
resource "aws_s3_bucket" "tf-test-2" {
  bucket = "techpro-test-2-locking-2"
}
```
...
```bash
scp -i ~/.ssh/<your pem file> <your pem file> ubuntu@<terraform instance public ip>:/home/ubuntu
```
...
resource "aws_instance" "instance" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("~/insbekir.pem")     # key.pem 
  }
...
ls
cat public_ip.txt
ssh -i ~/techpro.pem ec2-user@public_ip
exit
```
