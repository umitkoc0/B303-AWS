...
- In this example, we have 2 `EC2 instances` (1 ubuntu, 1 Amazon linux 2023), and a `security group` in our AWS account and we want to import these resources to our infrastructure.
...
- Create two EC2 instances one's ami is `Amazon Linux 3` and the other is `Ubuntu 24.04`. Their tags will be `Name=aws-linux-3`, `Name=ubuntu-24.04` and `Name=tf-import-sg`.  Attach security group to these instances.
...
variable "tf-tags" {
  type = list(string)
  default = ["aws-linux-3", "ubuntu-24.04", "red-hat-linux-9"]
}
...
- Go to the terraform registry and check the terraform aws instance import section. Then go to the AWS console, take the instance-id and import the "aws-linux-3" to the terraform management.

```bash
terraform import "aws_instance.tf-instances[0]" i-0f011ecae19add8ec
...
- Check the terraform.tfstate and import the second instance. (Take the ubuntu 24.04 instance-id from the console)

```bash
terraform import "aws_instance.tf-instances[1]" i-0890d48bb8944e1cf
...
```bash
terraform import aws_security_group.tf-sg sg-07cadd4f6369c248c
...
- Compare main.tf with .tfstate and edit main.tf according to .tfstate.
