...
### Create a module to create an aws instance with amazon linux 2023 ami.

- Create a directory for modules to publish.

```bash
cd && mkdir techproed-modules && cd techproed-modules && touch main.tf variables.tf outputs.tf versions.tf userdata.sh README.md .gitignore
```
...
- Go to the `main.tf` and prepare a config file to create an aws intance with ubuntu ami.
...
---
Terraform Module to provision an AWS EC2 instance with the ubuntu ami and installed docker in it.
...
module "docker_instance" {
    source = "<github-username>/docker-ec2/aws"
    key_name = "techpro"
}
```
---
...
- Create a `public` github repo and name it as `terraform-aws-docker-ec2`.
...
git clone https://github.com/USERACCOUNT/terraform-aws-docker-ec2.git
```
...
- Go to the `Terraform Registry` and sign in with your `Github Account`.(https://registry.terraform.io/)
...
- Website: https://registry.terraform.io/modules/<account>/docker-ec2/aws/latest
```
...
```bash
cd && mkdir techpro-modules && cd techpro-modules && touch main.tf
```
...
- After checking the instance, you can terminate it.(docker version and docker compose version)
